...
struct ContentView: View {
    ...
    var body: some View {
        ...
        NavigationView {
                ...
                ForEach(orders) { order in
                    HStack {
                        VStack(alignment: .leading) {
                            ...
                            Text("Table \(order.tableNumber)").font(.subheadline)
                        }
                        Spacer()
            ...
            .navigationBarItems(trailing: Button(action: {showOrderSheet = true}, label: {Image(systemName: "plus.circle").imageScale(.large)}))
            .sheet(isPresented: $showOrderSheet) {OrderSheet()}
        }
    }
